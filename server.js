...
app.put("/update/:database/:collection/:id", async (req, res) => {
  try {
    ...
    const updatedDocument = Model.findByIdAndUpdate(id, data, {
      new: true,
      runValidators: true,
    });
    ...
    if (!updatedDocument) {
      return res
        .status(404)
        .json({ message: "An error occured, the document was not found" });
    }
    // Log a success message to the console
    // Send back the updated document with a 200 status code
    res.status(200).json({ message: "Document was made successfully" });
  } catch (err) {
    // Log error to the console
    console.error("Something went wrong with POST route");
  ...
  try {
    ...
    if (!deletedDocument) {
      return res.status(404).json({
        message: "An error has occured, the document could not be deleted.",
      });
    }
    ...
    res.status(200).json({ message: "Document was successfully deleted." });
  } catch (err) {
    // Log error to the console
